...
The heavy extract_info call runs in a thread with a timeout so the bot
process keeps answering other users.
"""
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    # store the URL under a short key (callback_data is limited to 64 bytes)
    ...
    if info.is_playlist:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [InlineKeyboardButton(f"⬇️ حمّل {count} فيديو (أفضل جودة)", callback_data=f"pl:{key}:best")],
        ...
        caption = (
            ...
            f"عدد الفيديوهات اللي هتتحمل: {count} (بحد أقصى {config.max_playlist_items})\n"
            "كل فيديو بيتعامل معاه لوحده — لو واحد فشل الباقي بيكمل."
        )
        await _send_preview(message, status, info.thumbnail, caption, keyboard)
        return

    duration = format_duration(info.duration)
    qualities = available_qualities(info.formats)  # hides tiers this video doesn't actually have
...
async def _send_preview(message, status, thumbnail_url, caption, keyboard) -> None:
    """Show the thumbnail (if any) as its own plain photo message, then the
    metadata + quality keyboard as a separate, ordinary *text* message.

    Crucial: the keyboard message must stay plain text. Every downstream
    status update (queue position, download/upload progress, final result)
    edits this same message with edit_text(), and Telegram only allows
    editing a photo message's *caption* (edit_caption), never turning it
    into text — attaching the caption+keyboard directly to the photo here
    would make every one of those later edits crash with
